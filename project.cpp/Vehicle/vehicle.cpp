...
void VehicleList::addVehicle(int driverID) {
    ...
    getline(cin, v->plateNumber);

    cout << "Vehicle Type (Bike/Car/Rickshaw): ";
    getline(cin, v->type);

    v->rating = 5.0; // new vehicles default rating
    v->next = head;
    head = v;
...
void VehicleList::updateVehicle(int driverID) {
    ...
    if (!v) {
        cout << "No vehicle found for this driver.\n";
        return;
    }
